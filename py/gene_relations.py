...
###########
# Name: print_relation
# Input: relation data to store in database
# Output: relation data in a tab-separated string
###########

def print_relation(docid, mid1, mid2, word1, word2, is_correct, feature, sent_text):
    print '\t'.join([docid, mid1, mid2, word1, word2, is_correct, feature, sent_text, "\\N"])

    if is_correct is not "\\N":
        print '\t'.join([docid, mid1, mid2, word1, word2, "\\N", feature, sent_text, "\\N"])
        ...
        if len(sent.words) <= 50:
            for w1 in genes:
                for w2 in genes:
                    ...
                    features = generate_features(doc, sent, w1, w2)
                    ...
                    supervise(doc, mid1, mid2, w1, w2, feature, sent_text, found_domain, flag_family, high_quality_verb):
                    
                    return
######
# Name: generate_features
# Input: Document object, Sentence object, two Word objects for genes
# Return: features array
#
# Generate features for two genes
######

def generate_features(doc, sent, w1, w2):
    ...
    for i in range(minindex+1, maxindex):
        ...
        if re.search('VB\w*', sent.words[i].pos): 
    ...
    negation_words = ["no", "not", "neither", "nor"]
    hq_verbs = ["interact", "associate", "bind", "regulate", "phosporylate", "phosphorylated"]
    high_quality_verb = 0
    if len(verbs_between) == 1 and neg_found == 0:
        features.append("SINGLE_VERB_BETWEEN_with[%s]" % verbs_between[0])
        if verbs_between[0] in hq_verbs:
            high_quality_verb = 1
    else:
    ...
    if len(ws) == 1 and ws[0] == "and" and minindex > 1:
        if minindex > 2:
            if sent.words[minindex - 3].lemma not in negation_words and \
    ...
    int_words = ["association", "interaction", "complex", "activation", "bind", "binding"]
    if minindex > 1:
        if sent.words[minindex - 2].lemma.lower() in int_words:
    ...
    negation_words = ["no", "not", "neither", "nor"]
    if sent.words[maxindex-1].lemma in negation_words:
        features.append("NEG_SECOND_GENE[%s]" % sent.words[maxindex - 1].lemma)

    if minindex > 0:
        if sent.words[minindex-1].lemma in negation_words:
            features.append("NEG_FIRST_GENE[%s]" % sent.words[minindex - 1].lemma)


    if mini_w2 == mini_w1 and mini_w1 != None and len(minp_w1) < 100: 
    ...
    found_plural = 0
    if minindex > 0:
        if sent.words[minindex + 1].pos in plural_pos: 
            found_plural = 1
    ...
    if maxindex < len(sent.words) - 1 and found_plural == 0:
        if sent.words[maxindex + 1].pos in plural_pos: 
            found_plural = 1
...
######
# Name: supervise
# Input: Document object, mention ids for gene1 and gene 2, feature array, sentence text, flags for domains
#    faimily, high quality verbs
# Return: prints relation to the database
#
# Distant supervision for two genes in a document, with their feature array
######

def supervise(doc, mid1, mid2, w1, w2, feature, sent_text, found_domain, flag_family, high_quality_verb):

    if w1.word in dict_exclude_dist_sup and w2.word in dict_exclude_dist_sup[w1.word]:
        print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "\\N", feature, sent_text) 

    elif sent.words[0].word == "Abbreviations" and sent.words[1].word == "used":
        if doc.docid.split(".pdf")[0] not in dict_gs_docids:
            print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "false", feature, sent_text) 
        else:
            print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "\\N", feature, sent_text) 
        ...
        if w1.word in dict_interact and w2.word in dict_interact[w1.word] and "mutation" not in sent_text and "mutations" not in sent_text and "variant" not in sent_text and "variants" not in sent_text and "polymorphism" not in sent_text and "polymorphisms" not in sent_text:
            ...
            if found_domain == 0 and flag_family == 0:
                if doc.docid.split(".pdf")[0] not in dict_gs_docids:
                    print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "true", feature, sent_text)
                else:
                    print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "\\N", feature, sent_text) 
            else:
                print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "\\N", feature, sent_text) 
            ...
            if w1.word in dict_no_interact and ("binds" not in ws and "interacts" not in ws and "interacted" not in ws and "bound" not in ws and "complex" not in ws and "associates" not in ws and "associated" not in ws and "bind" not in ws and "interact" not in ws):
                ...
                if doc.docid.split(".pdf")[0] not in dict_gs_docids:
                    ...
                    print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "\\N", feature, sent_text) 
            elif no_interact_phrase == True and high_quality_verb == 0: 
                if doc.docid.split(".pdf")[0] not in dict_gs_docids:
                    print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "false", feature, sent_text) 
                else:
                    print_relation(doc.docid, mid1, mid2, w1.word, w2.word, "\\N", feature, sent_text) 
